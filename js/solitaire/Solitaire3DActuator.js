// 3D Actuator for Nature's Grove: Respite (Solitaire Pai Sho)
// Extends PaiSho3DActuator with Solitaire-specific game rendering
...
export class Solitaire3DActuator extends PaiSho3DActuator {
	...
	getHostTilesContainerDivs() {
		return SolitaireController.getHostTilesContainerDivs();
	}

	getGuestTilesContainerDivs() {
		return SolitaireController.getGuestTilesContainerDivs();
	}
	...
	// --- Override actuate to match Solitaire's signature ---
	// Solitaire GameManager calls: actuate(board, theGame, markingManager, drawnTile)
	...
	addBoardPoint3D(boardPoint) {
		...
		if (tile.drained || tile.trapped) {
			this.addDrainedIndicator(tileGroup);
		}
