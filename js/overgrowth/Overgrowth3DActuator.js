// 3D Actuator for Nature's Grove: Overgrowth
// Extends PaiSho3DActuator with Overgrowth-specific game rendering
...
export class Overgrowth3DActuator extends PaiSho3DActuator {
	...
	getHostTilesContainerDivs() {
		return OvergrowthController.getHostTilesContainerDivs();
	}

	getGuestTilesContainerDivs() {
		return OvergrowthController.getGuestTilesContainerDivs();
	}
	...
	// --- Override actuate to match Overgrowth's signature ---
	// Overgrowth GameManager calls: actuate(board, theGame, markingManager, drawnTile)
	...
	addBoardPoint3D(boardPoint) {
		...
		// Harmony/clash glow (Overgrowth uses HOST for harmony, GUEST for clash)
		if (tile.inHarmony) {
