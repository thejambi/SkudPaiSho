// 3D Actuator for Spirit Pai Sho
// Extends PaiSho3DActuator with Spirit-specific game rendering
...
export class Spirit3DActuator extends PaiSho3DActuator {
	...
	getHostTilesContainerDivs() {
		return SpiritController.getHostTilesContainerDivs();
	}

	getGuestTilesContainerDivs() {
		return SpiritController.getGuestTilesContainerDivs();
	}
	...
	// --- Override actuate to match Spirit's 3-param signature ---
	// Spirit GameManager calls: actuate(board, tileManager, moveToAnimate)
	// Base class expects: actuate(board, tileManager, markingManager, moveToAnimate, moveAnimationBeginStep)

	actuate(board, tileManager, moveToAnimate) {
		super.actuate(board, tileManager, null, moveToAnimate, 0);
	}
	...
	render3DGame(board, tileManager, markingManager, moveToAnimate, moveAnimationBeginStep) {
		...
		// Tile piles (Spirit uses named container divs like Skud)
